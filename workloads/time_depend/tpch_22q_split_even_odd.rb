...
NoSE::TimeDependWorkload.new do
  ...
  Group 'Even', default: frequencies.reverse do
    ...
    # l_shipdate の cardinality は 2526, l_receiptdate の cardinality は 2554 のため，僅差ではあるが，l_receiptdate を使用
     Q 'SELECT lineitem.l_shipmode, sum(l_orderkey.o_orderpriority), lineitem.l_commitdate, lineitem.l_shipdate '\
  ...
  Group 'Odd', default: frequencies do
    ...
    # l_shipdate の cardinality は 2526, o_orderdate のカーディナリティは 2406 のため，o_orderdate を使用．
    ...
    # p_size, l_quantity の cardinailty が共に50のため，p_size を使用
    Q 'SELECT sum(lineitem.l_extendedprice), sum(lineitem.l_discount), lineitem.l_quantity ' \
      'FROM lineitem.l_partkey.ps_partkey ' \
    ...
    # l_receiptdate の cardinality は 2554, l_commitdate の cardinality は 2466 ため，l_receiptdate を使用
