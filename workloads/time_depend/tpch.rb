...
NoSE::TimeDependWorkload.new do
  ...
  #doubled = (0...12).map{|i| 100 * (2 ** i)}
  doubled = (0...5).map{|i| 10 ** i}
  #doubled = (0...6).map{|i| 10 * (10 ** i)}

  TimeSteps doubled.size
  Interval 3600 # specify interval in minutes
  ...
  Group 'Upseart', default: doubled do
    Q 'INSERT INTO lineitem SET l_orderkey=?, l_linenumber=?, l_quantity=?, l_extendedprice=?, l_discount=?, ' \
                  ...
                  'l_shipmode=?, l_comment=?, dummy=? AND CONNECT TO l_partkey(?), l_orderkey(?) -- 1'
    Q 'INSERT INTO orders SET o_orderkey=?, o_orderstatus=?, o_totalprice=?, o_orderdate=?, o_orderpriority=?, '\
                  'o_clerk=?, o_shippriority=?, o_comment=?, dummy=? AND CONNECT TO from_lineitem(?), o_custkey(?) -- 4'
  end
  ...
  'GROUP BY c_nationkey.n_name -- Q5'
    ...
    #Q 'SELECT l_orderkey.o_orderdate, from_lineitem.l_extendedprice, from_lineitem.l_discount, to_nation.n_name '\
    #  'FROM part.from_partsupp.from_lineitem.l_orderkey.o_custkey.c_nationkey.n_regionkey ' \
    #  'WHERE n_regionkey.r_name = ? AND l_orderkey.o_orderdate < ? AND l_orderkey.o_orderdate > ? AND part.p_type = ? ' \
