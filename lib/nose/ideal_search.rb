  ...
  module Search
    ...
    class IdealSearch < Search
      ...
      def get_migrate_preparing_plans(query_trees, indexes)
        ...
        #show_migrate_plans(migrate_plans)
        migrate_plans
      end

      def show_migrate_plans(migrate_plans)
        migrate_plans.each do |target_index, m_plan|
          puts "==========================================="
          puts "target_index: #{target_index.key}: #{target_index.hash_str}"
          m_plan.each do |query, cost_tree|
            if query.instance_of? MigrateSupportQuery
              puts MigrateSupportQuery.name
              puts cost_tree[:tree].inspect
            end
          end
          m_plan.each do |query, cost_tree|
            if query.instance_of? MigrateSupportSimplifiedQuery
              puts MigrateSupportSimplifiedQuery.name
              puts cost_tree[:tree].inspect
            end
          end
          puts "==========================================="
      ...
      def populate_query_costs(query_costs, steps_by_index, weight,
        ...
        steps_by_index.each do |steps|
          ...
          if query_costs.key? index_step.index
            ...
            # TODO ここで場合によっては上書きされるけど、イテレートの順序を固定していないので、結果に再現性が無い原因になってない?
            # ここの下の処理に入る前に steps_by_index をコストでソートしたらどうか
            # 結果的に採用されるのが１つだけであることを考えると、下の方法で上書きするだけでも良い?
            unless is_same_cost current_cost, cost
              puts "cost overlapping #{index_step.index.key}"
              if is_larger_cost current_cost, cost
                # if the new cost value is bigger than the current value,
                # delete newly created query plan
                #puts "step removed for : #{index_step.index.key}"
                ...
                # delete costly plan and update the cost value
                #planner.prune_plan query_costs[index_step.index].first.first
                #puts "cost is overwritten for : #{index_step.index.key}"
